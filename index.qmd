...
```{r}
rpart_classes <- rpart(spam ~ ., dfTrain, method = "class")
```
...
rpart_classes <- predict(rpart_fit, dfa_test, type = 'class')
```
...
fit_2 <- NN3 %>% 
  ...
  set_hparams(p = ncol(dfa_test), q1 = 10, q2 = 10, o = 3) %>%
  set_opt_hparams(lr = 0.001) %>%
  fit(
    data = list(
      dfa %>% select(x1, x2) %>% as.matrix,
